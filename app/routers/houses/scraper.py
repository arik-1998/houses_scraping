...
def get_house_info(url, headers, db):
    ...
    if short_house_info is None:
        print(url)
        global flag_for_log
        if flag_for_log:
            with open("logs/log_" + url.split("/")[4] + "_" + str(
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + ".txt", "w") as file:
                file.write(request_text)
            flag_for_log = False
        return None
    adress = soup.find(class_="loc").text if soup.find(class_="loc") else None
    price = soup.find(class_="price x").text if soup.find(class_="price x") else None
    ...
    if soup.find_all(class_="attr g"):
        land_info = soup.find_all(class_="attr g")[1].find(class_="i").text if soup.find_all(class_="attr g")[
            1] else None
